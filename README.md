## **🔮AdaMind**

An intelligent and interactive learning platform that generates **topic-based quizzes and flashcards instantly**.  
Designed to make learning faster, smarter, and more engaging for students, educators, and self-learners.

---

## 🚀 Features

### 🔹 AI-Powered Quiz Generation
- Enter any topic  
- Automatically generates **5 high-quality questions** based on your input  
- Useful for quick assessments or revision  

### 🔹 Saved Quizzes
- Save generated quizzes for later  
- Easy access for repeated practice  

### 🔹 Flashcard Generator
- Create flashcards instantly from any topic  
- Perfect for fast revision and spaced repetition  

### 🔹 Saved Flashcards
- Store and manage all previously created flashcards  
- Organized and accessible anytime  

---

## 🛠️ Tech Stack
- **Frontend:** React.js, CSS, JavaScript  
- **Backend:** Node.js / Express  
- **Database:** MongoDB  
- **Other Tools:** REST APIs, JWT  

---

## 📂 Project Structure
/client → Frontend  
/server → Backend  
/database → SQL schema and queries  

---

## 📸 Screenshots 
- Home Page  
<img width="1700" height="830" alt="AdaMind Home Page screenshot" src="https://github.com/ArfaKA-03/assets/blob/main/home.jpg" />
- Quiz Generation UI  
<img width="1300" height="800" alt="AdaMind Quiz screenshot" src="https://github.com/ArfaKA-03/assets/blob/main/quiz.jpg" />
- Flashcard View
<img width="1859" height="849" alt="AdaMind Flashcard screenshot" src="https://github.com/ArfaKA-03/assets/blob/main/flashcard.jpg" />
- Saved Items Dashboard  
<img width="1859" height="849" alt="AdaMind Progress screenshot" src="https://github.com/ArfaKA-03/assets/blob/main/progress.jpg" />
-Profile
<img width="1859" height="849" alt="AdaMind Profile screenshot" src="https://github.com/ArfaKA-03/assets/blob/main/profile.jpg" />


---

## 🧩 How It Works
1. User selects **Generate Quiz**  
2. Enters a topic (e.g., “Operating Systems”, "React Hooks")  
3. System generates **5 topic-based questions**  
4. User can save quiz or turn it into flashcards  
5. Saved items are accessible anytime from the dashboard  

---
## 🏗️ Installation & Setup

### 1. Clone the Repository
```bash
git clone https://github.com/your-username/your-repo-name.git
cd your-repo-name
```
